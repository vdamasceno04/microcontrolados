...
GetMasterPassword
	...
	LDR R4,=ENTER_MASTER_PASSWORD_STR ; muda a string que vai pro display
...
SetNewMasterPassword
	...
	CMP R7, #8					; Verifica se 4 digitos foram inseridos
	BLT SetNewMasterPassword			; Se não, volta para o inicio	
	BHI GetMasterPassword
	BEQ ChangedMasterPassword

ChangedMasterPassword
	MOV R7, #0
	BL LCD_Reset
	LDR R4,=CHANGED_MASTER_PASSWORD_STR ; muda a string que vai pro display
	BL LCD_PrintString ;imprime nova string
	MOV R0, #2000		 ; seta o tempo de 2s
	BL SysTick_Wait1ms
	POP {LR}
...
ENTER_MASTER_PASSWORD_STR DCB "Nova senha mestr", 0
CHANGED_MASTER_PASSWORD_STR DCB "Senha mestre alt", 0
